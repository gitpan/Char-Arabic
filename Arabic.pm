...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Earabic::function_ord     = 'Arabic::ord'     if /\A ord \z/oxms;
                    $Earabic::function_ord_    = 'Arabic::ord_'    if /\A ord \z/oxms;
                    $Earabic::function_reverse = 'Arabic::reverse' if /\A reverse \z/oxms;
                    $Earabic::function_getc    = 'Arabic::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Arabic::escape();
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Arabic software provides)

Arabic software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Arabic software provide)
...
=head1 Un-Escaping bytes::* Subroutines (Arabic software provides)

Arabic software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Arabic software and
