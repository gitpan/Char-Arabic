...
######################################################################
#
# Arabic - Source code filter to escape Arabic script
#
...
my $function_ord;         # ord()   to ord()   or Arabic::ord()
my $function_ord_;        # ord     to ord     or Arabic::ord_
my $function_reverse;     # reverse to reverse or Arabic::reverse
my $function_getc;        # getc    to getc    or Arabic::getc
...
END
    ...
    print Arabic::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    my $fh = gensym();
    Earabic::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Arabic::escape_script {
    ...
    # read Arabic script
    my $fh = gensym();
    Earabic::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Earabic(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Earabic '%s.0'; # 'quote' for perl5.6\n", $Arabic::VERSION); # require run-time routines version

        # use Arabic version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        $function_getc    = 'getc';
        if (s/^ \s* use \s+ Arabic \s* ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Arabic::VERSION) {
                    my @file = grep -e, map {qq{$_/Arabic.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Arabic.pm $version, but @{[__FILE__]} is version $Arabic::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Earabic.pm $version, but \\\$Earabic::VERSION is \$Earabic::VERSION" if \$Earabic::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Arabic::VERSION) {
                    die "Script $0 required Arabic.pm $version, but @{[__FILE__]} is only version $Arabic::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Arabic::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Arabic::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Arabic::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Arabic::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Arabic::re_t=%s; while($Arabic::re_t =~ %s){%s local $^W=0; local $Arabic::re_r=%s; %s$Arabic::re_t="$`$Arabic::re_r$'"; pos($Arabic::re_t)=length "$`$Arabic::re_r"; } return $Arabic::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Arabic::re_r=eval $Arabic::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Arabic::re_r=%s; %s"%s$Arabic::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Arabic::re_n=0; while(%s =~ %s){%s local $^W=0; local $Arabic::re_r=%s; %s%s="$`$Arabic::re_r$'"; pos(%s)=length "$`$Arabic::re_r"; $Arabic::re_n++} return %s$Arabic::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Arabic::re_r=%s; %s%s="%s$Arabic::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Arabic::re_r=%s; %s%s="%s$Arabic::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Arabic::re_r=eval $Arabic::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Arabic.pm and Earabic.pm and put it in your perl lib directory.

=item * You'll need to write "use Arabic;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Arabic;
  use Arabic ver.sion;             --- require minimum version
  use Arabic ver.sion.0;           --- expects version (match or die)

  # "no Arabic;" not supported

  or

  $ perl Arabic.pm Arabic_script.pl > Escaped_script.pl.e
...
Arabic software is "middleware" between perl interpreter and your Perl script
written in Arabic.
...
By "use Arabic;", it automatically interpret your script as Arabic. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Arabic.pm, Earabic.pm
  +---------------------------------------+
...
import of Earabic.pm works well.
...
=head1 Software Composition
   ...
   Arabic.pm               --- source code filter to escape Arabic
   Earabic.pm              --- run-time routines for Arabic.pm
...
You need write 'use Arabic;' in your script.
...
=head1 Calling 'Earabic::ignorecase()' (Arabic.pm provides)

Arabic.pm applies calling 'Earabic::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Arabic.pm provides)

Arabic.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Arabic.pm and Earabic.pm provide)
...
=head1 Un-Escaping bytes::* Subroutines (Arabic.pm provides)

Arabic.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/Arabic and change
'use utf8;' to 'use Arabic;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Earabic.pm provides these
  functions.
...
Arabic software remains one language and one interpreter.
