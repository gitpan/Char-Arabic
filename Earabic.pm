...
BEGIN {
    ...
    sub qualify ($;$) {
        my ($name) = @_;
        if (!ref($name) && (Earabic::index($name, '::') == -1) && (Earabic::index($name, "'") == -1)) {
            my $pkg;
...
sub Earabic::rindex($$;$);
sub Earabic::lcfirst(@);
sub Earabic::lcfirst_();
sub Earabic::lc(@);
sub Earabic::lc_();
sub Earabic::ucfirst(@);
sub Earabic::ucfirst_();
sub Earabic::uc(@);
sub Earabic::uc_();
sub Earabic::fc(@);
sub Earabic::fc_();
sub Earabic::ignorecase;
sub Earabic::classic_character_class;
sub Earabic::capture;
sub Earabic::chr(;$);
sub Earabic::chr_();
sub Earabic::glob($);
...
${Earabic::not_lower_i} = qr{(?:[\x00-\xFF])};
...
${Earabic::not_upper_i} = qr{(?:[\x00-\xFF])};
...
sub Earabic::lcfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Earabic::lc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Earabic::lc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Earabic::lc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
...
sub Earabic::ucfirst(@) {
    if (@_) {
        my $s = shift @_;
        if (@_ and wantarray) {
            return Earabic::uc(CORE::substr($s,0,1)) . CORE::substr($s,1), @_;
        }
        else {
            return Earabic::uc(CORE::substr($s,0,1)) . CORE::substr($s,1);
        }
    }
    else {
        return Earabic::uc(CORE::substr($_,0,1)) . CORE::substr($_,1);
    }
}
    ...
    for my $string (@string) {
        ...
        for (my $i=0; $i <= $#char; $i++) {
            ...
            # with /i modifier
                ...
                my $uc = Earabic::uc($char[$i]);
                my $fc = Earabic::fc($char[$i]);
                if ($uc ne $fc) {
                    if (CORE::length($fc) == 1) {
                        $char[$i] = '['   . $uc       . $fc . ']';
                    }
            ...
            for (@singleoctet) {
                while (s/ \A \\x(..) - \\x(..) //oxms or s/ \A \\x((..)) //oxms) {
                    for my $ord (hex($1) .. hex($2)) {
                        ...
                        my $uc = Earabic::uc($char);
                        my $fc = Earabic::fc($char);
                        if ($uc eq $fc) {
...
sub Arabic::getc(;*@) {
    ...
    croak 'Too many arguments for Arabic::getc' if @_ and not wantarray;
    ...
    for my $length ($length[0] .. $length[-1]) {
        ...
        if (exists $range_tr{CORE::length($getc)}) {
            if ($getc =~ /\A ${Earabic::dot_s} \z/oxms) {
...
sub Arabic::index($$;$) {
    ...
    my $index;
    if (@_ == 3) {
        $index = Earabic::index($_[0], $_[1], CORE::length(Arabic::substr($_[0], 0, $_[2])));
    }
    else {
        $index = Earabic::index($_[0], $_[1]);
    }
    ...
    if ($index == -1) {
        ...
        return Arabic::length(CORE::substr $_[0], 0, $index);
    }
}
...
sub Arabic::rindex($$;$) {
    ...
    my $rindex;
    if (@_ == 3) {
        $rindex = Earabic::rindex($_[0], $_[1], CORE::length(Arabic::substr($_[0], 0, $_[2])));
    }
    else {
        $rindex = Earabic::rindex($_[0], $_[1]);
    }
    ...
    if ($rindex == -1) {
        ...
        return Arabic::length(CORE::substr $_[0], 0, $rindex);
    }
}
...
my $qq_substr  = qr{(?: Arabic::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
# subroutines of package Earabic
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Earabic::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Earabic::glob';              }
    elsif (/\G \b lc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Earabic::lc_';               }
    elsif (/\G \b lcfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Earabic::lcfirst_';          }
    elsif (/\G \b uc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Earabic::uc_';               }
    elsif (/\G \b ucfirst \b       (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Earabic::ucfirst_';          }
    elsif (/\G \b fc \b            (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Earabic::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Earabic::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Earabic::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Earabic::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Earabic::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Earabic::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Earabic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Earabic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Earabic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Earabic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Earabic::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Earabic::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Earabic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Earabic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Earabic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Earabic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Earabic::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Earabic::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Earabic::split' . $e . qq     {" "};  }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> @{[Earabic::PREMATCH()]}
            ...
            $e_string .= q{Earabic::PREMATCH()};
            $slash = 'div';
        }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> @{[Earabic::MATCH()]}
        elsif ($string =~ /\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
            $e_string .= q{Earabic::MATCH()};
            $slash = 'div';
        }
...
# subroutines of package Earabic
        ...
        elsif ($string =~ /\G \b Arabic::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Arabic::eval \b                                /oxgc) { $e_string .= 'eval Arabic::escape'; $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::substr \b                             /oxgc) { $e_string .= 'substr';            $slash = 'm//'; }
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Earabic::chop';       $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';             $slash = 'm//'; }
        elsif ($string =~ /\G \b Arabic::index \b                               /oxgc) { $e_string .= 'Arabic::index';       $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Earabic::index';      $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';            $slash = 'm//'; }
        elsif ($string =~ /\G \b Arabic::rindex \b                              /oxgc) { $e_string .= 'Arabic::rindex';      $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Earabic::rindex';     $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Earabic::lc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Earabic::lcfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Earabic::uc';         $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Earabic::ucfirst';    $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Earabic::fc';         $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Earabic::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Earabic::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Earabic::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Earabic::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Earabic::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Earabic::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Earabic::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Earabic::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Earabic::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Earabic::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Earabic::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Earabic::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Earabic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Earabic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Earabic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Earabic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Earabic::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Earabic::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Earabic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Earabic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Earabic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Earabic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Earabic::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Earabic::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Earabic::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        ...
        return Earabic::classic_character_class($char);
    }
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Earabic::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Earabic::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Earabic::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Earabic::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Earabic::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Earabic::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Earabic::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Earabic::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Earabic::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Earabic::ignorecase(Earabic::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Earabic::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Earabic::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Earabic::ignorecase(Earabic::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Earabic::MATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Arabic::re_t=%s; while($Arabic::re_t =~ %s){%s local $^W=0; local $Arabic::re_r=%s; %s$Arabic::re_t="$`$Arabic::re_r$'"; pos($Arabic::re_t)=length "$`$Arabic::re_r"; } return $Arabic::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Arabic::re_r=CORE::eval $Arabic::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Arabic::re_r=%s; %s"%s$Arabic::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Arabic::re_n=0; while(%s =~ %s){%s local $^W=0; local $Arabic::re_r=%s; %s%s="$`$Arabic::re_r$'"; pos(%s)=length "$`$Arabic::re_r"; $Arabic::re_n++} return %s$Arabic::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Arabic::re_r=%s; %s%s="%s$Arabic::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Arabic::re_r=%s; %s%s="%s$Arabic::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Arabic::re_r=CORE::eval $Arabic::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
This Earabic module first appeared in ActivePerl Build 522 Built under
...
=item Split string
  ...
  @split = Earabic::split(/pattern/,$string,$limit);
  @split = Earabic::split(/pattern/,$string);
  @split = Earabic::split(/pattern/);
  @split = Earabic::split('',$string,$limit);
  @split = Earabic::split('',$string);
  @split = Earabic::split('');
  @split = Earabic::split();
  @split = Earabic::split;
  ...
  at all, Earabic::split returns the original string as a single substring, If it
  ...
  @chars  = Earabic::split(//,  $word);
  @fields = Earabic::split(/:/, $line);
  @words  = Earabic::split(" ", $paragraph);
  @lines  = Earabic::split(/^/, $buffer);
  ...
  print join(":" => Earabic::split(/ */, "hi there"));
  ...
  Earabic::split's pattern is exempt from that wrinkle.)

  The $limit parameter splits only part of a string:

  my ($login, $passwd, $remainder) = Earabic::split(/:/, $_, 3);
  ...
  splits on whitespace just as Earabic::split with no arguments does. Thus,
  Earabic::split(" ") can be used to emulate awk's default behavior. In contrast,
  Earabic::split(/ /) will give you as many null initial fields as there are
  ...
  $string = join(" ", Earabic::split(" ", $string));
  ...
  %head = ("FRONTSTUFF", Earabic::split(/^(\S*?):\s*/m, $header));
  ...
  while (<>) {
      for my $word (Earabic::split()) {
          $count{$word}++;
      }
  }

  The inverse of Earabic::split is join, except that join can only join with the
...
=item Transliteration
  ...
  $tr = Earabic::tr($variable,$bind_operator,$searchlist,$replacementlist,$modifier);
  $tr = Earabic::tr($variable,$bind_operator,$searchlist,$replacementlist);
  ...
  print Earabic::tr('bookkeeper','=~','boep','peob','r'); # prints 'peekkoobor'

=item Chop string

  $chop = Earabic::chop(@list);
  $chop = Earabic::chop();
  $chop = Earabic::chop;

  This subroutine chops off the last character of a string variable and returns the
  character chopped. The Earabic::chop subroutine is used primary to remove the newline
  ...
  since Earabic::chop always shortens the string no matter what's there, and chomp
  is more selective. If no argument is given, the subroutine chops the $_ variable.

  You cannot Earabic::chop a literal, only a variable. If you Earabic::chop a list of
  variables, each string in the list is chopped:

  @lines = `cat myfile`;
  Earabic::chop(@lines);

  You can Earabic::chop anything that is an lvalue, including an assignment:

  Earabic::chop($cwd = `pwd`);
  Earabic::chop($answer = <STDIN>);

  This is different from:

  $answer = Earabic::chop($tmp = <STDIN>); # WRONG

  which puts a newline into $answer because Earabic::chop returns the character
  ...
  In the most general case, Earabic::chop can be expressed using substr:

  $last_code = Earabic::chop($var);
  $last_code = substr($var, -1, 1, ""); # same thing

  Once you understand this equivalence, you can use it to do bigger chops. To
  Earabic::chop more than one character, use substr as an lvalue, assigning a null
  ...
  form of substr, creating something of a quintuple Earabic::chop;
...
=item Index string
  ...
  $byte_pos = Earabic::index($string,$substr,$byte_offset);
  $byte_pos = Earabic::index($string,$substr);
  ...
  $byte_pos = -1;
  while (($byte_pos = Earabic::index($string, $lookfor, $byte_pos)) > -1) {
...
=item Reverse index string
  ...
  $byte_pos = Earabic::rindex($string,$substr,$byte_offset);
  $byte_pos = Earabic::rindex($string,$substr);

  This subroutine works just like Earabic::index except that it returns the byte
  position of the last occurrence of $substring in $string (a reverse Earabic::index).
  ...
  while (($byte_pos = Arabic::rindex($string, $lookfor, $byte_pos)) >= 0) {
      print "Found at $byte_pos\n";
      $byte_pos--;
  }
...
=item Lower case string
  ...
  You can use the Earabic::fc subroutine for case-insensitive comparisons via Arabic
  software.

=item Lower case first character of string

  $lcfirst = Earabic::lcfirst($string);
  $lcfirst = Earabic::lcfirst_;
...
=item Upper case string
  ...
  in interpolated strings. For titlecase, use Earabic::ucfirst instead.
...
=item Upper case first character of string
  ...
  $ucfirst = Earabic::ucfirst($string);
  $ucfirst = Earabic::ucfirst_;
...
=item Fold case string
  ...
  $fc = Earabic::fc($string);
  $fc = Earabic::fc_;

  New to Arabic software, this subroutine returns the full Unicode-like casefold of
  ...
  Earabic::fc($a) eq Earabic::fc($b)

  The reliable way to compare string case-insensitively was with the /i pattern
  modifier, because Arabic software has always used casefolding semantics for
...
=item Make ignore case string
  ...
  @ignorecase = Earabic::ignorecase(@string);

  This subroutine is internal use to m/ /i, s/ / /i, split / /i, and qr/ /i.

=item Make capture number

  $capturenumber = Earabic::capture($string);

  This subroutine is internal use to m/ /, s/ / /, split / /, and qr/ /.

=item Make character

  $chr = Earabic::chr($code);
  $chr = Earabic::chr_;

  This subroutine returns a programmer-visible character, character represented by
  that $code in the character set. For example, Earabic::chr(65) is "A" in either
  ASCII or Arabic, not Unicode. For the reverse of Earabic::chr, use Arabic::ord.

=item Filename expansion (globbing)

  @glob = Earabic::glob($string);
  @glob = Earabic::glob_;
  ...
  The Earabic::glob subroutine grandfathers the use of whitespace to separate multiple
  ...
  @spacies = <"*e f*">;
  @spacies = Earabic::glob('"*e f*"');
  @spacies = Earabic::glob(q("*e f*"));

  If you had to get a variable through, you could do this:

  @spacies = Earabic::glob("'*${var}e f*'");
  @spacies = Earabic::glob(qq("*${var}e f*"));
