...
#
# http://search.cpan.org/dist/Char-Char::Arabic/
#
...
BEGIN { CORE::require Char::Earabic; }
...
my $function_ord;         # ord()   to ord()   or Char::Arabic::ord()
my $function_ord_;        # ord     to ord     or Char::Arabic::ord_
my $function_reverse;     # reverse to reverse or Char::Arabic::reverse
my $function_getc;        # getc    to getc    or Char::Arabic::getc
...
perl $0 Arabic_script.pl > Escaped_script.pl.e
END
    }

    print Char::Arabic::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Earabic::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::Arabic::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Earabic::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::Arabic::escape_script {
    ...
    # read Arabic script
    my $fh = gensym();
    Char::Earabic::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Earabic(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        $e_script .= sprintf("use Char::Earabic %s.0;\n", $Char::Arabic::VERSION); # require run-time routines version

        # use Char::Arabic version qw(ord reverse getc);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        $function_getc    = 'getc';
        if (s/^ \s* use \s+ Char::Arabic \s* ([^;]*) ; \s* \n? $//oxms) {

            # require version
            my $list = $1;
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::Arabic::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/Arabic.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/Arabic.pm $version, but @{[__FILE__]} is version $Char::Arabic::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Earabic.pm $version, but \\\$Char::Earabic::VERSION is \$Char::Earabic::VERSION" if \$Char::Earabic::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::Arabic::VERSION) {
                    die "Script $0 required Char/Arabic.pm $version, but @{[__FILE__]} is only version $Char::Arabic::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::Arabic::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::Arabic::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::Arabic::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::Arabic::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::Arabic::re_t=%s; while($Char::Arabic::re_t =~ %s){%s local $^W=0; local $Char::Arabic::re_r=%s; %s$Char::Arabic::re_t="$`$Char::Arabic::re_r$'"; pos($Char::Arabic::re_t)=length "$`$Char::Arabic::re_r"; } return $Char::Arabic::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Arabic::re_r=eval $Char::Arabic::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::Arabic::re_r=%s; %s"%s$Char::Arabic::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::Arabic::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Arabic::re_r=%s; %s%s="$`$Char::Arabic::re_r$'"; pos(%s)=length "$`$Char::Arabic::re_r"; $Char::Arabic::re_n++} return %s$Char::Arabic::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::Arabic::re_r=%s; %s%s="%s$Char::Arabic::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::Arabic::re_r=%s; %s%s="%s$Char::Arabic::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Arabic::re_r=eval $Char::Arabic::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
=item * You'll have to download Char/Arabic.pm and Char/Earabic.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::Arabic;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::Arabic;
  use Char::Arabic ver.sion;             --- require minimum version
  use Char::Arabic ver.sion.0;           --- expects version (match or die)

  # "no Char::Arabic;" not supported

  or

  $ perl Char/Arabic.pm Arabic_script.pl > Escaped_script.pl.e

  then

  $ perl Escaped_script.pl.e

  Arabic_script.pl  --- script written in Arabic
...
Char::Arabic software is "middleware" between perl interpreter and your Perl script
written in Arabic.

Perl is optimized for problems which are about 90% working with text and about
10% everything else. Even if this "text" doesn't contain Arabic, Perl3 or later
can treat Arabic as binary data.

By "use Char::Arabic;", it automatically interpret your script as Arabic. The various
functions of perl including a regular expression can treat Arabic now.
...
This software is a source code filter to escape Perl script encoded by Arabic
...
=item * Remains One Language Handling Raw Arabic, Doesn't Use UTF8 flag like JPerl
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/Arabic.pm, Char/Earabic.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Earabic.pm
works well.
...
=head1 Software Composition
   ...
   Char/Arabic.pm               --- source code filter to escape Arabic
   Char/Earabic.pm              --- run-time routines for Char/Arabic.pm
...
You need write 'use Char::Arabic;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::Arabic;
  ---------------------------------

=head1 Calling 'Char::Earabic::ignorecase()' (Char/Arabic.pm provides)

Char/Arabic.pm applies calling 'Char::Earabic::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Earabic::ignorecase($var)]}.../
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Earabic.pm provides)
...
Definitions in Char/Earabic.pm.
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/Arabic.pm provides)

Char/Arabic.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/Arabic.pm and Char/Earabic.pm provide)

Insert 'Char::Earabic::' at head of function name. Char/Earabic.pm provides your script Char::Earabic::*
subroutines.

  -------------------------------------------
  Before      After            Works as
  -------------------------------------------
  lc          Char::Earabic::lc        Character
  lcfirst     Char::Earabic::lcfirst   Character
  uc          Char::Earabic::uc        Character
  ucfirst     Char::Earabic::ucfirst   Character
  fc          Char::Earabic::fc        Character
  chr         Char::Earabic::chr       Character
  glob        Char::Earabic::glob      Character
  -------------------------------------------
...
=head1 Un-Escaping bytes::* Subroutines (Char/Arabic.pm provides)

Char/Arabic.pm removes 'bytes::' at head of subroutine name.
...
You need copy built-in standard module to /Perl/site/lib/Char::Arabic and change
'use utf8;' to 'use Char::Arabic;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Earabic.pm provides these
  functions.
...
Char::Arabic software remains one language and one interpreter.
